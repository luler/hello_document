...
class PageHelper
{
    ...
    public function autoPage($page = 'page', $page_rows = 'page_rows')
    {
        if ($page !== false) {
            ...
            if (request()->param($page) && is_numeric(request()->param($page))) {
                $this->page = request()->param($page);
            }
            if (request()->param($page_rows) && is_numeric(request()->param($page_rows))) {
                $this->page_rows = request()->param($page_rows);
            }
